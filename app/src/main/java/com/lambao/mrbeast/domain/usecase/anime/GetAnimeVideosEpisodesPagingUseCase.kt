...
class GetAnimeVideosEpisodesPagingUseCase @Inject constructor(
    private val animeRepository: AnimeRepository,
    dispatcherProvider: DispatcherProvider
) : PagingUseCase<AnimeParams, AnimeVideoEpisode>(dispatcherProvider) {
    override fun execute(params: AnimeParams?) = flow {
        if (params == null) {
            throw ParamsEmptyException()
        } else {
            emitAll(animeRepository.getAnimeVideosEpisodesPaginated(params))
        }
    }
}