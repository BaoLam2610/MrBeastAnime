...
class GetAnimeCharactersPagingUseCase @Inject constructor(
    private val animeRepository: AnimeRepository,
    dispatcherProvider: DispatcherProvider
) : PagingUseCase<AnimeParams, AnimeCharacter>(dispatcherProvider) {
    override fun execute(params: AnimeParams?) = flow {
        if (params == null) {
            throw ParamsEmptyException()
        } else {
            emitAll(animeRepository.getAnimeCharactersPaginated(params))
        }
    }
}